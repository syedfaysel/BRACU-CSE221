// @syedrajo20
// task : 2
#include <bits/stdc++.h>

#define nl "\n";
#define all(a) a.begin(), a.end()
#define fast ios_base::sync_with_stdio(0); cin.tie(0)
#define file freopen("input2.txt", "r", stdin);freopen("output2.txt", "w", stdout)

using namespace std;
typedef long long int ll;
const int N = 1e5+10;
const int INF = 1e9+10;

int main() {

    fast; // faster I/O
    #ifndef EPSILON
        file; // handles I/O files
    #endif
    // solution code starts here...

    int n, m; cin >> n >> m;

    vector<pair<int, int>> task, selected;
    vector<int> current;

    for (int i = 0; i < n; i++){
        int a, b; cin >> a >> b;
        task.push_back(make_pair(b, a));
    }

    sort(task.begin(), task.end());

    int  prev;
    for (int i = 0; i < n; i++){
        if (selected.size() == 0){
            selected.push_back(task[0]);
            current.push_back(task[0].first);

        }
        // set prev to the finish time of the previous selected task

        prev = selected.back().first;
        if(prev <= task[i].second){
            selected.push_back(task[i]);
            current.push_back(task[i].first);
        }
        else{

            if (current.front() <= task[i].second){
                current.erase(current.begin());
                current.push_back(task[i].first);
                selected.push_back(task[i]);
            }
            else if (current.size() < m ){
                current.push_back(task[i].first);
                selected.push_back(task[i]);
            }
        }
    }

    cout << selected.size() << "\n";


    return 0;
}
